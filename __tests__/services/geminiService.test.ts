...
const mockCreateChat = jest.fn(() => mockChat);
...
describe('geminiService', () => {
    ...
    describe('analyzeVideoWithFrames', () => {
        it('should return analysis text on successful API call', async () => {
            ...
            mockGenerateContent.mockResolvedValue({ text: 'Analysis successful' } as any);
        ...
        it('should throw ApiKeyError for invalid API key message', async () => {
            ...
            mockGenerateContent.mockRejectedValue(new Error('API key not valid') as any);
        ...
        it('should retry on 503 error and succeed on the second attempt', async () => {
            ...
            mockGenerateContent
                // Fix: Cast mock rejected value to 'any' to avoid TypeScript type inference issues with Jest mocks.
                .mockRejectedValueOnce(new Error('503 overloaded') as any)
                // Fix: Cast mock resolved value to 'any' to avoid TypeScript type inference issues with Jest mocks.
                .mockResolvedValueOnce({ text: 'Success on retry' } as any);
        ...
        it('should throw a user-friendly error after max retries on 503 errors', async () => {
            // Fix: Cast mock rejected value to 'any' to avoid TypeScript type inference issues with Jest mocks.
            mockGenerateContent.mockRejectedValue(new Error('503 unavailable') as any);
            
            const promise = analyzeVideoWithFrames('prompt', ['frame1']);
            
            await jest.advanceTimersByTimeAsync(2000); // 1st retry
            await jest.advanceTimersByTimeAsync(4000); // 2nd retry
        ...
        it('should throw a generic error for non-retryable failures', async () => {
            // Fix: Cast mock rejected value to 'any' to avoid TypeScript type inference issues with Jest mocks.
            mockGenerateContent.mockRejectedValue(new Error('Some other error') as any);
    ...
    describe('generateWorkoutPlan', () => {
        it('should return a parsed workout plan on success', async () => {
            ...
            // Fix: Cast mock resolved value to 'any' to avoid TypeScript type inference issues with Jest mocks.
            mockGenerateContent.mockResolvedValue({ text: JSON.stringify(mockPlan) } as any);
        ...
        it('should include regeneration instructions in the prompt when requested', async () => {
            // Fix: Cast mock resolved value to 'any' to avoid TypeScript type inference issues with Jest mocks.
            mockGenerateContent.mockResolvedValue({ text: '[]' } as any);
            await generateWorkoutPlan('barbell', 'Advanced', 'Lose Fat', undefined, true);
            // Fix: Cast the argument to 'any' to access its properties in the test.
            const prompt = (mockGenerateContent.mock.calls[0][0] as any).contents.parts[0].text;
        ...
        it('should throw an error if JSON parsing fails', async () => {
            // Fix: Cast mock resolved value to 'any' to avoid TypeScript type inference issues with Jest mocks.
            mockGenerateContent.mockResolvedValue({ text: 'this is not json' } as any);
    ...
    describe('getGroundedAnswer', () => {
        it('should return text and sources on success', async () => {
            ...
            // Fix: Cast mock resolved value to 'any' to avoid TypeScript type inference issues with Jest mocks.
            mockGenerateContent.mockResolvedValue(mockResponse as any);
        ...
        it('should throw a specific error for safety blocks', async () => {
            // Fix: Cast mock resolved value to 'any' to avoid TypeScript type inference issues with Jest mocks.
            mockGenerateContent.mockResolvedValue({ text: '', candidates: [{ finishReason: 'SAFETY' }] } as any);
    ...
    describe('analyzePose', () => {
        it('should return text analysis on success', async () => {
            // Fix: Cast mock resolved value to 'any' to avoid TypeScript type inference issues with Jest mocks.
            mockGenerateContent.mockResolvedValue({ text: 'Your form is great!' } as any);
    ...
    describe('editImage', () => {
        ...
        it('should throw if no image is returned', async () => {
            // Fix: Cast mock resolved value to 'any' to avoid TypeScript type inference issues with Jest mocks.
            mockGenerateContent.mockResolvedValue({ candidates: [{ content: { parts: [{ text: 'I cannot do that.' }] } }] } as any);
    ...
    describe('getQuickChatResponse', () => {
        it('should return a quick text response on success', async () => {
            // Fix: Cast mock resolved value to 'any' to avoid TypeScript type inference issues with Jest mocks.
            mockGenerateContent.mockResolvedValue({ text: 'A quick reply.' } as any);
        ...
        it('should fail silently and return an empty string on error', async () => {
            // Fix: Cast mock rejected value to 'any' to avoid TypeScript type inference issues with Jest mocks.
            mockGenerateContent.mockRejectedValue(new Error('Model failed') as any);
            const result = await getQuickChatResponse('A quick question');
            expect(result).toBe('');
        });
    });
    ...
    describe('analyzeFoodImage', () => {
        it('should return an array of food items on success', async () => {
            ...
            // Fix: Cast mock resolved value to 'any' to avoid TypeScript type inference issues with Jest mocks.
            mockGenerateContent.mockResolvedValue({ text: JSON.stringify(mockFoods) } as any);
    ...
    describe('getNutritionalAnalysis', () => {
        it('should return a full nutritional info object on success', async () => {
            ...
            // Fix: Cast mock resolved value to 'any' to avoid TypeScript type inference issues with Jest mocks.
            mockGenerateContent.mockResolvedValue({ text: JSON.stringify(mockNutrition) } as any);
    ...
    describe('findYouTubeVideoForExercise', () => {
        it('should extract and return a valid YouTube URL', async () => {
            // Fix: Cast mock resolved value to 'any' to avoid TypeScript type inference issues with Jest mocks.
            mockGenerateContent.mockResolvedValue({ text: 'Here is the video: https://www.youtube.com/watch?v=xyz123' } as any);
        ...
        it('should return null if no URL is found in the response', async () => {
            // Fix: Cast mock resolved value to 'any' to avoid TypeScript type inference issues with Jest mocks.
            mockGenerateContent.mockResolvedValue({ text: 'Sorry, I could not find a video.' } as any);
        ...
        it('should return null on API error', async () => {
            // Fix: Cast mock rejected value to 'any' to avoid TypeScript type inference issues with Jest mocks.
            mockGenerateContent.mockRejectedValue(new Error('API failed') as any);
    ...
    describe('transcribeAudio', () => {
        it('should return transcribed text on success', async () => {
            // Fix: Cast mock resolved value to 'any' to avoid TypeScript type inference issues with Jest mocks.
            mockGenerateContent.mockResolvedValue({ text: 'This is a transcription.' } as any);
    ...
    describe('getQuickResponse', () => {
        it('should return a text response on success', async () => {
            // Fix: Cast mock resolved value to 'any' to avoid TypeScript type inference issues with Jest mocks.
            mockGenerateContent.mockResolvedValue({ text: 'Quick answer.' } as any);
